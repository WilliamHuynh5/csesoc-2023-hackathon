...
export default function Login() {
  ...
  const handleOnSubmit = async (event) => {
    ...
    try {
      const res = await post("http://localhost:49152/register", {
        email,
        password,
      }, false);
  ...
  return (
    <div className="h-screen relative flex flex-col gap-4 justify-center items-center w-full bg-gradient-to-b from-white to-slate-100">
      ...
      <p>Already have an account? <Link className="text-blue-500 duration-150 hover:underline" href="/login">Login</Link></p>
